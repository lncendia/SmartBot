    ...
    : IRequestHandler<SetPositionCommand>
{
    ...
    /// <summary>
    /// Сообщение с инструкцией для ввода вечернего отчёта.
    /// </summary>
    private const string EveningReportMessage =
        "<b>Добрый вечер! 🌇</b>\n\n" +
        "Сейчас время для <b>вечернего отчёта</b>. Утренний отчёт уже не актуален, так как вы зарегистрировались после 18:00.\n\n" +
        "Пожалуйста, составьте <b>вечерний отчёт</b>, чтобы подвести итоги дня. Опишите:\n" +
        "1. Что удалось сделать.\n" +
        "2. Какие возникли сложности.\n" +
        "3. Какие задачи переносятся на завтра.\n\n" +
        "<i>Пример:</i>\n" +
        "<code>Сегодня выполнил подготовку отчёта по продажам, но не успел проанализировать данные по новым клиентам. Переношу эту задачу на завтра.</code>\n\n" +
        "Если вы не хотите вносить вечерний отчёт за сегодня, просто подождите до <b>следующего рабочего дня</b>.";
    ...
    public async Task Handle(SetPositionCommand request, CancellationToken cancellationToken)
    {
        ...
        // Отправляем сообщение с инструкцией для ввода утреннего отчёта
        if (currentTime.IsEveningReportTime())
        {
            await client.SendMessage(
                chatId: request.ChatId,
        ...
        // Отправляем сообщение с инструкцией для ввода вечернего отчёта
        else if(currentTime.IsEveningReportTime())
        {
            await client.SendMessage(
                chatId: request.ChatId,
